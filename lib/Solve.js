...
const [IUAMHTML, invisibleHTML, TurnstileHTML, RecaptchaHTML] =
  await Promise.all([
...
const generateInvisibleHTML = (r, t) =>
  invisibleHTML.replace("![r]!", r).replace("![t]!", t);
const generateTurnstileHTML = (sitekey) =>
  TurnstileHTML.replace("![sitekey]!", sitekey);
const generateRecaptchaHTML = (sitekey, action) =>
  RecaptchaHTML.replace(/!\[sitekey\]!/g, sitekey).replace(
    "![action]!",
    action,
  );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (
          ...
          !invalidTypes.includes(data.type) ||
          !data.url ||
        ...
        this.proxyUrl =
          data.proxy?.scheme && data.proxy?.host && data.proxy?.port
            ? `${data.proxy.scheme}://${data.proxy.username && data.proxy.password ? `${data.proxy.username}:${data.proxy.password}@` : ""}${data.proxy.host}:${data.proxy.port}`
            : undefined;
        ...
        if (data.type === "V3" || data.type === "IUAM") {
          ...
          if (
            ...
            resolve({
              ...
              code: 400,
              cerror: html.err ? html.err : undefined,
              errormessage: `Failed to find IUAM challenge. Check the URL or proxy. (${typeof html} ${typeof html.response} ${html.status} ${html.response})`,
            });
            return;
          }
        ...
        resolve({
          ...
          code: 500,
          errormessage:
            "An error occurred on our side. Please check your request or try again later.",
          error,
        });
        return;
      }
