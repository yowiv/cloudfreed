    ...
    try {
      if (typeof mode !== "string") {
        return resolve({
          success: false,
          code: 400,
          errormessage: "Invalid mode entered, please check your parameters.",
        });
      }
      ...
      if (
        ...
        return resolve({
          success: false,
          code: 400,
          errormessage: "Invalid proxy entered, please check your parameters.",
        });
      }
      ...
      if (typeof url === "string") {
        ...
        return resolve({
          success: false,
          code: 500,
          errormessage: "Invalid URL entered, please check your parameters.",
        });
      }
        ...
        try {
          if (code !== 0) {
            return resolve({
              ...
              err,
              errormessage:
                "Error occurred when requesting from URL, please check URL/proxy.",
            });
          }
          ...
          if (typeof proxy === "string") {
            if (proxy.startsWith("socks5://")) {
              while (outs.length > 2 && !outs[0].includes("HTTP/")) {
                outs.shift();
              }
            } else {
              outs.shift();
          ...
          if (!headers || !headers.includes("HTTP/")) {
            return resolve({
              ...
              code: 500,
              err,
              errormessage: "Invalid response headers received",
            });
          }
          ...
          const [responseStatusCode, responseUrl] = [
            parseInt(data.split("|")[0], 10),
            data.split("|")[1],
          ]; // Remove and parse the last line as the status code
          ...
          return resolve({
            ...
            err,
            error,
            errormessage: "Error occurred while resolving curl.",
          });
        }
      });
      ...
      curl.on("error", (error) => {
        return resolve({
          ...
          code: 500,
          error,
          errormessage: "Error occurred while running curl.",
        });
      });
    } catch (error) {
      return resolve({
        ...
        code: 500,
        error,
        errormessage: "Error occurred while starting curl.",
      });
    }
  });
}
